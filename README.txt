...
DOS_Restore is itself a Console Win32 program. 
...
If a target file exists its writes the new file with a new version number.
I.e. is my_file.txt exists it is restored as my_file_(1).txt and if  my_file_(1).txt then the restored file is my_file_(2).txt.
This process continue until a new name is found that does not exist.

In essence CONTROL.001 is a list of records that are either directories or files in those directories.
